...
- デバッグを成功させるための二つの条件：システムがどのように動作するはずであるかを知る。データを収集し、原因の仮説を立て、理論を検証する
- 小規模で新しいシステムを扱う場合には、ウマ（一般的なバグ）を想定する。古く大規模で、比較的安定したシステムを扱う場合には、シマウマ（稀なバグ）を想定する。
- 重大なインシデントの場合には、デバッガと調査担当者を分単位の対応から切り離し、集中できるスペースを用意する
- 運用過負荷とアラート疲れは、盲点を生むことになる。逸脱の常態化に対処するには、新たなチームメンバーの意見を積極的に聞き、オンコールローテーションや対応チームの人員を適宜入れ替えて新鮮な視点を取り入れることが有用。
- 相関関係は因果関係ではないものの、特にサービス障害開始時点で発生する相関関係は重要かつ調査に効果的であることが多い
- コードのバグ箇所を厳密に把握できない場合は、コード品質への一般的な改善が役に立つ可能性がある
  - ユニットテストのカバレッジ改善、並行処理プログラミングにおける差に対座、エラー処理の改善（コンテキストの追加など）
- 機密情報を扱わないシステムでも、アクセスと認可など標準のセキュリティ制御を取り入れることで、将来のバグの防止と検査に役立つ
- 共同デバッグは、デバッグの経験共有と心理的安全性の構築に役立つ
  - ノートPCを開くのは二人だけ、操作者は他の人から要求されたアクションを実行する。記録者は作業を記録する。全てのアクションは同席する参加者によって決定される。操作者と記録者は取るべきアクションを決めない
  - デイリーで前日のアラートに対してチームでモブデバッグの時間など取れると良さそう
- デバッグがよりコードに焦点を合わせるのに対して、セキュリティ調査は攻撃の背後にいる敵に素早く焦点を合わせる必要がある
- バグ（障害）の大部分は実際にはバグであるかもしれないが、とはいえ悪意のあるアクションにも警戒を怠らない
- コアダンプ、メモリダンプ、スタックトレースなどのシステム情報は、可能な限りログと同様に扱う必要がある
- ログ管理におけるプライバシーへの配慮：ロギングの深さ、保持、アクセスと監査、データの匿名化や仮名化、暗号化
- [Cloud Access Security Broker (CASB)](https://support.google.com/a/answer/10490409?hl=en) は、エンドユーザーとクラウドサービスの仲介役として、セキュリティ制御を適用してロギングを提供する
- ログのバジェット管理戦略：S/N 比が良いログを重視する。ログの圧縮。ストレージをウォームとコールドに分ける。インテリジェントなローテーションを実施する

## 第４部：システムの維持

### 16章：ディザスタプランニング

- ディザスタの定義：自然災害、インフラに関する障害、サービスやプロダクトの障害、外部の攻撃者、機密データの無許可の開示、緊急のセキュリティ脆弱性
- ディザスタへの対応：緊急対応、短期復旧、長期復旧、業務再開
- ディザスタリスク分析
  - 損傷するか、オンフラインになると業務を遂行できなくなる可能性が高いシステム。システムは「業務に不可欠」、「業務に重要」、「必要性が低い」に分類できる
  - インシデント対応に必要な、技術的または人的リソース
  - 個別のシステムで発生する可能性があるディザスタのシナリオ。シナリオは発生の可能性、発生頻度、業務への影響でグルーピングできる
- インシデントの重大度は、一旦確定されたらインシデントのライフサイクルを通じて固定されるのが一般的。対して優先度はインシデント対応の過程で変化することもある
- インシデント対応チームの稼働パラメータ：インシデントの初期対応に要する想定時間、初期トリアージ評価の実施と対応計画および稼働予定の作成に要する想定時間、サービスレベル目標
- 詳細な手順書：特定のタスクを一貫して実行する方法を記載した、対応計画を補完する資料。一時的な管理アクセスを許可したり、特定のログを抽出する方法の手順など。チームやサービスごとに整備することもある。
- インシデントに先立つ準備
  - システムの設定：ローカルシステムへの耐障害性への導入、フォレンジックエージェントの有効化、バックアップのリストア手順の確立と演習、緊急時のアクセスおよびリカバリ用のフォールバックパス
  - エスカレーションや意思決定、情報整理などの訓練
  - ハードウェアやソフトウェアの調達方法、アウトソーシングサービスの承認などに関するプロセスの確立
- 全社ミーティングの枠を使ってインシデント対応手順のクイズとかやっても良いかも  
- システムと対応計画のテスト：自動化システムの評価、対応プロセスのテストと評価などが必要
- 机上演習はプロダクション環境に影響を与えずに、インシデント対応をシミュレートする。迫真性、具体性、判断ポイント、参加者と進行役、結果とフォードバックが必要となる
- プロダクション環境を用いてテストする際には、フォルトインジェクションを用いることで個々のチームが依存関係を関与させずにシステムをテストできる
- インシデントとテストに対する評価：対応を計測する。非難のないポストモーテムを書き、システムやプロセスの改善方法に焦点を当てる。フォレンジック分析を実施して、ギャップに対処できるように適切なログと他の関連資料を保存する、など
- Google のグローバル規模のインシデント対応計画、さすがに想定する事象のスケールと解像度が高い（自然災害時にオフィスにいるツアーメンバーや、従業員の家庭対応と業務との利益相反など）
