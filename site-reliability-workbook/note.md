...
- Maglev はフロントエンドサーバー群に対する TCP レベルの負荷分散を行う。
- Google Front End (GFE) は、Google のサービスにおいて入力トラフィックを処理するシステム
- 例えば GKE でロードバランサーをデプロイすると、内部的には GFE や Maglev がその処理を担うことになる
- Pokemon Go では元々 Network Load Balancer を使っていたが、想定以上のトラフィックを効率的に処理するために Google Cloud Load Balancer に移行し、大規模なネットワーク上で運用されるプロキシを利用した。
  - 当初はフロントエンドプロキシへのリクエストが多すぎたため、接続が拒否され、インバウンドリクエストのモニタリングでキャッチできない事態もあった
- オートスケーリングにおける不健全なインスタンスの管理は、ロードバランシングやインスタンスのレディネスチェック、オートヒールなどの戦略を組み合わせることで改善できる
- オートスケーラーがスケーリングの基準とする要素にバグ（例えば CPU が意図せず使われたり、依存先のサービスに不具合があったり、など）があると予期しないスケーリングが発生し得る。スケーリングの上限と下限を設定するのが重要。
- オートスケールの不具合に対処するために、オンコールエンジニアはオートスケーリングの無効化と、手動スケーリングの方法を整備しておくと良い。
- 同一プロジェクトで複数のスケーリングを設定する場合は、それぞれのスケーリングが共有するリソースクオータを消費し尽くさないように注意が必要
- オートスケーリング、ロードバランシング、ロードシェディングは、どれもシステムの負荷を均等化して安定させるという共通の目的がある。
- それぞれの負荷調整アプローチを組み合わせるには、事前に関係を把握しておくことが大事
  - ロードバランサー：リクエストが一つの場所に集中している場合は、場所ごとに最初元のインスタンス数を設定することで、フェールオーバー時の余剰キャパシティを保持しておける
  - ロードシェディング：ロードシェディングが働く前に、オートスケールするように閾値の設定が必要
  - RPC での負荷調整：過度に時間がかかっているリクエストや、すでにエラーを返しているリクエストなどを早期にキャンセルすることで、全体として効率的に処理を実行できる。

### １２章：非抽象的な大規模システム設計の紹介

- システムの設計と実装において、低い運用コストで頑健かつスケーラブルな設計を行うためのイテレーティブなスタイル：非抽象的な大規模システム設計（NALSD）
- NALSD: 高可用性のシステムに必要な、キャパシティプランニング、コンポーネントの分離、システムのグレースフルデグラデーションの組み合わせとなる
- Google SRE には、システムの構成図からリソースプランニングを始め、スケーリングや障害について考え設計を具体的なリソースの提案にできることが求められる
- 現実的なシステムを設計する上では、しっかりとした理由付けと仮定の置き方が重要。これによる妥当な結果を組み合わせて、設計の理解を深める
- NALSDの第１フェーズ：基本の設計として、原理的にうまく働く設計を考える
  - 設計として実現可能か？
  - もっと上手（高速、小さい、効率的）いやり方はないか？
- 第２フェーズ：基本設計のスケールアップ
  - 現実的か？
  - 弾力性があるか？どんな障害が起こるとどのような挙動になるか？
  - もっと上手いやり方はあるか？
- SLO は NALSD の現実性を考える参考指標としても使うことができる
- NALSD 面白い。システム設計におけるフェルミ推定みたいな。処理が必要なトラフィックやデータ処理能力を概算する能力身につけたい。Nucleus, Catalyst とかでも考えてみるべきなのかも。
...
- リリースエンジニアリング：リポジトリから取り出したコードが運用中のプロダクションシステムへ到達するまでに関連する、すべてのプロセスや成果物を表す言葉
- リリース自動化によって、リリース時のトイルの排除やプロセスの一貫性、ロールアウトの正確な状況把握、ロールバックなどを目指す
- カナリアは、実質的には A/B テストのプロセスと言える
- リリースエンジニアリングの基本原則：再現可能なビルド、自動化されたビルド、自動化されたテスト、自動化されたデプロイ、小規模なデプロイ
- メリット：トイル削減によるエンジニアの作業軽減、レビューとバージョン管理の強制、再現可能な自動化によるミスの減少、リリースパイプラインのモニタリングが可能になる
- リリースは根本的にサービスに不確実性をもたらすが、SLO やエラーバジェットを用いることで、信頼性に対するリリースのインパクトを計測できる
- カナリアプロセスの要件：カナリアの変更をサービスの一部にデプロイする方法、カナリアの変更の良し悪しの評価プロセス、リリースプロセスへのカナリア評価の統合
  - 良いカナリア：高い信頼性で悪いリリース候補を検出できる、同時に偽陽性なしに良いリリースを特定できる
  - 最初はロールバックの方法あたりからインプリしていくと、取り組み推進中に障害があっても影響を抑えることができて良いかも
- カナリアの比率が実際の可用性と SLO の比率程度に十分小さければ、自信を持ってカナリアを継続できる（カナリアリリースによる障害が起こっても、SLO を大きく毀損するほどにはならない）
- 実施するカナリアデプロイは、一度に一つだけが良い。並行数が増えるとシステムの状態追跡の労力が大きくなる。
- カナリアの影響要素：サイズと期間、トラフィックの量、一日の中の時間、メトリクス
  - トラフィックが特定の期間に集中するような場合、トラフィックが少ない間のリリースが適切かどうかはどう判断する？判断に必要な分量のリクエストを待つことが、次のリリースへのブロッカーになるかもしれない
- カナリアの良し悪しを判断するのに適したメトリクス：まずはそのサービスの SLI から始めるのがベター。加えて、ユーザー影響との関係が強い順に数個を選ぶと良い。
- カナリアとコントロールシステムの隔離が不十分な場合、カナリアデプロイによる動作がコントロール群に負のインパクトを与える可能性がある。システムが正しく評価していることを確かめるには、SLI のような絶対的な計測値を用いるのも良い。
- 非インタラクティブなサービス（パイプラインなど）では、カナリアとコントロールの隔離やメトリクスの計算が複雑になりやすいので注意
- カナリア運用時のモニタリングは、カナリアとコントロールでメトリクスを分割して集計するのが良い
- 人工的なトラフィックによる検証はステージングまでで実施すれば良さそう。ティーイングはちょっとレベル高いかも。
